...
class Journal:
    ...
    def FindType(self):
        find_in_scopus = self.scopu_data[
            self.scopu_data["Title name"].apply(lambda x: x.upper())
            == self.journal_name.upper()
        ]
        if not find_in_scopus.empty:
            return "Scopus"

        find_in_scie = self.scie_data[
            self.scie_data["Journal title"].apply(lambda x: x.upper())
            == self.journal_name.upper()
        ]
    ...
    def scopu_data(self):
        return pd.read_excel(
            self.scopus_path, sheet_name="Accepted titles Feb. 2024", skiprows=2
        )
